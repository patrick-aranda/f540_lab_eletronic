...
with open("ch2_fourier_dados.csv", "r") as file:
    csvreader = csv.reader(file)
    next(csvreader) #next()method returns me the next record from my variable. Since is the first time used, it returns the first record (first line) and skip to next one.
    next(csvreader)
    for row in csvreader: #"row" poderia ser qlq coisa, é apenas o índice da linha.
        aux = []
        aux.extend(row) #extendo minha linha, deixando cada termo da linha em uma coluna. Como antes dei um next, csvreader começa da 2ª linha.
        columns[2].append(float(aux[1])) #adiciono o i-ézimo termo da minha lista aux à minha i-ézima coluna.
